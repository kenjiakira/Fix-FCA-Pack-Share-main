...
def run(phone, i):
    ...
    print(f"Spam thanh cong lan {i}")
    for j in range(4, 0, -1):
        print(f"Vui long cho {j} giay", end="\r")
        time.sleep(1)

if __name__ == "__main__":
    # Nhập số điện thoại và số lần chạy từ người dùng
    import sys
    
    if len(sys.argv) != 3:
        print("Usage: python dec.py <phone> <count>")
        sys.exit(1)
        
    phone = sys.argv[1]
    count = int(sys.argv[2])

    # Lặp lại theo số lần người dùng nhập
