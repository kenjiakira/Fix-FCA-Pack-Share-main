const { getBalance, updateBalance, updateQuestProgress, loadQuy, saveQuy } = require('../utils/currencies');
...
module.exports = {
    ...
    generateResult: function(senderID, playerChoice, betType, balance) {
        ...
        const result = shouldWin ? playerChoice : (playerChoice === "up" ? "ngửa" : "up");
    ...
    onLaunch: async function({ api, event, target = [] }) {
        try {
            ...
            if (!["up", "ngửa"].includes(choice)) {
                return api.sendMessage("Vui lòng chọn 'up' hoặc 'ngửa'.", threadID, messageID);
            }
            ...
            if (this.lastPlayed[senderID] && currentTime - this.lastPlayed[senderID] < 90000) {
                return api.sendMessage(`Vui lòng đợi ${Math.ceil((90000 - (currentTime - this.lastPlayed[senderID])) / 1000)} giây nữa.`, threadID, messageID);
            }
            ...
            setTimeout(async () => {
                try {
                    ...
                    let message = `Kết quả: ${COIN_FACES[result]} (${result.toUpperCase()})\n`;
