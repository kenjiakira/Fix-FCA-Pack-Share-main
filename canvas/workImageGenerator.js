...
async function createWorkResultImage(result, vipBenefits, senderID) {
    try {
        ...
        registerFont(path.join(assetsPath, 'Montserrat-Bold.ttf'), { family: 'Montserrat', weight: 'bold' });
        registerFont(path.join(assetsPath, 'Montserrat-SemiBold.ttf'), { family: 'Montserrat', weight: '600' });
        registerFont(path.join(assetsPath, 'Montserrat-Medium.ttf'), { family: 'Montserrat', weight: '500' });
        registerFont(path.join(assetsPath, 'Montserrat-Regular.ttf'), { family: 'Montserrat', weight: 'normal' });
        ...
        const glassGradient = ctx.createLinearGradient(0, cardStartY, 0, cardStartY + 100);
        glassGradient.addColorStop(0, 'rgba(255, 255, 255, 0.15)');
        glassGradient.addColorStop(1, 'rgba(255, 255, 255, 0.05)');
        ctx.fillStyle = glassGradient;
        roundedRect(ctx, 30, cardStartY, width - 60, 100, { tl: 20, tr: 20, br: 0, bl: 0 });
        ...
        const lineGradient = ctx.createLinearGradient(80, cardStartY + 80, width - 80, cardStartY + 80);
...
function findJobCategory(jobId) {
    return Object.entries(require('../config/family/jobConfig').JOB_CATEGORIES).find(([_, cat]) => 
        cat.jobs.includes(jobId)
    )?.[1];
}
...
function drawGoldenCorners(ctx, x, y, width, height) {
    ...
    const goldGradient = ctx.createLinearGradient(x, y, x + cornerLength, y + cornerLength);
...
function drawLevelSection(ctx, width, levelY, result) {
    ...
    const shieldGradient = ctx.createLinearGradient(iconX, iconY, iconX + iconSize, iconY + iconSize);
    ...
    if (nextRank) {
        ...
        if (progressPercent < 1) {
            roundedRect(ctx, barX, progressY, barWidth * progressPercent, barHeight, 
                        {tl: 11, tr: 0, br: 0, bl: 11});
        } else {
            roundedRect(ctx, barX, progressY, barWidth * progressPercent, barHeight, 11);
        }
        ...
        ctx.textAlign = 'center';
        ctx.fillText(`${currentProgress}/${worksNeeded}`, width / 2, progressY + 16);
        
        // Next rank information
        ctx.font = '500 18px Montserrat';
        ctx.fillStyle = '#E0E0E0';
        ctx.fillText(`Cấp tiếp theo: ${nextRank.name} (+${((nextRank.bonus - 1) * 100).toFixed(0)}% lương)`, 
                    width / 2, progressY + 50);
    } else {
        // Max level message
        ctx.font = 'bold 24px Montserrat';
        const maxLevelGradient = ctx.createLinearGradient(width/2 - 120, levelY + 160, width/2 + 120, levelY + 160);
        ...
        ctx.font = '500 18px Montserrat';
        ctx.fillStyle = '#E0E0E0';
        ctx.fillText(`Thưởng hiện tại: +${((ranks[ranks.length - 1].bonus - 1) * 100).toFixed(0)}% lương`, 
                    width / 2, levelY + 190);
    }
}
...
function drawEarningsSection(ctx, width, earningsY, result) {
    ...
    const titleGradient = ctx.createLinearGradient(width/2 - 100, earningsY, width/2 + 100, earningsY);
    titleGradient.addColorStop(0, '#ffffff');
    titleGradient.addColorStop(0.5, '#f8e483');
    titleGradient.addColorStop(1, '#ffffff');
    ctx.fillStyle = titleGradient;
    ctx.fillText('KẾT QUẢ LÀM VIỆC', width / 2, earningsY + 20);
    ...
    // Income information
    const infoItems = [
        { icon: '💰', label: 'Được trả', value: `${result.salary.toLocaleString('vi-VN')} Xu`, color: '#ffffff' },
        { icon: '📝', label: 'Thuế thu nhập', value: `${tax.toLocaleString('vi-VN')} Xu (${(result.tax || 0).toFixed(1)}%)`, color: '#ff7675' },
        { icon: '✨', label: 'Thực lãnh', value: `${netEarnings.toLocaleString('vi-VN')} Xu`, color: '#2ecc71' }
    ];
...
function drawVIPBonusSection(ctx, width, vipY, result, vipBenefits) {
    ...
    const vipGradient = ctx.createLinearGradient(badgeX, vipY, badgeX + badgeWidth, vipY);
    ...
    ctx.font = '500 20px Montserrat';
    ctx.fillStyle = '#E0E0E0';
    ctx.fillText(`Thưởng VIP: +${vipBenefits.workBonus}%`, width / 2, bonusY + 25);
    
    ctx.font = 'bold 22px Montserrat';
    ctx.fillStyle = '#FFD700';
    ctx.fillText(`+${vipBonus.toLocaleString('vi-VN')} Xu`, width / 2, bonusY + 50);
...
function drawCooldownSection(ctx, width, cooldownY, cooldown) {
    ...
    ctx.textAlign = 'center';
    ctx.fillText(`${cooldownHours > 0 ? `${cooldownHours} giờ ` : ''}${cooldownMinutes} phút`, width / 2, cooldownY + 65);
}
...
function drawLevelUpOverlay(ctx, width, height, levelUpInfo) {
    ...
    const levelUpGradient = ctx.createLinearGradient(width/2 - 150, bannerY + 150, width/2 + 150, bannerY + 150);
    ...
    ctx.fillStyle = '#ffffff';
    ctx.fillText(`Thưởng: +${((levelUpInfo.bonus - 1) * 100).toFixed(0)}% lương`, width / 2, bannerY + 220);
...
function drawShield(ctx, x, y, size) {
    ...
    ctx.quadraticCurveTo(
        innerX + innerWidth / 2, innerY + innerHeight * 1.3,
        innerX, innerY + innerHeight * 0.6
    ); // Bottom curve
